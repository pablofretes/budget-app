...
export class MovementService {
  ...
  async findById(id: number): Promise<Movement> {
    ...
    if (!movement)
      throw new NotFoundException(ERROR_MESSAGES.MOVEMENT_NOT_FOUND);
    return movement;
  }

  async createMovement(movementDto: CreateMovementDto): Promise<Movement> {
    const balance: Balance = await this.balanceRepository.findOneBy({
      id: movementDto.balanceId,
    });
