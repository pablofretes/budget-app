...
import { MOVEMENT_CONCEPT, MOVEMENT_TYPE, PROVIDERS, RESPONSE_MESSAGES } from "../../../common/constants";
...
describe("BalanceService", () => {
	...
	it("should create update a movement", async () => {
		...
		const createdMovement = await movementService.updateMovement(id, movementDto);
		expect(createdMovement).toEqual({
			balanceId: expect.any(Number),
			amount: movementDto.amount,
			type: movementDto.type,
